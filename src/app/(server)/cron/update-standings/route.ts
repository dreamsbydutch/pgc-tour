...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!tourCards || tourCards.length === 0) {
      console.log("No tour cards found for current season");
      return NextResponse.redirect(`${origin}/`);
    }

    // Step 1: Calculate statistics for each tour card
    tourCards = await Promise.all(tourCards.map(calculateTourCardStats));

    // Step 2: Calculate standings positions
    calculateStandingsPositions(tourCards);

    // Step 3: Update all tour cards in database
    await updateTourCardsInDatabase(tourCards);

    // Step 4: Notify cache invalidation
    await notifyCacheInvalidation(origin);

    console.log(`✅ Updated standings for ${tourCards.length} tour cards`);
  } catch (error) {
...
// http://localhost:3000/cron/update-standings
