...
export function TabletListing({
  ...
  return (
    <div
      ...
      className={cn("mx-auto my-0.5 max-w-4xl rounded-md text-center")}
    >
      <div
        className={cn(
          "grid-cols-20 grid grid-flow-row py-0.5",
          type === "PGC" &&
      ...
      >
        <div className="col-span-3 flex place-self-center font-varela text-base">
          {team.position}
        ...
        <div className="col-span-6 place-self-center font-varela text-lg">
          {type === "PGA" ? golfer?.playerName : userTeam?.tourCard.displayName}
        </div>
        <div className="col-span-3 place-self-center font-varela text-base">
        ...
        team.position === "DQ" ? (
          ...
          <>
            ...
            <div className="col-span-2 place-self-center whitespace-nowrap font-varela text-sm">
              {team.thru === 18 ? "F" : team.thru}
              {type === "PGA" && golfer?.endHole === 9 ? "*" : ""}
            </div>
          </>
        )}
        <div className="place-self-center font-varela text-xs">
          {team.roundOne}
        </div>
        <div className="place-self-center font-varela text-xs">
          {team.roundTwo}
      ...
      {isOpen && golfer && type == "PGA" && (
        <PGADropdown {...{ golfer, userTeam, total }} />
      )}
      ...
      <div className="col-span-10 mb-4 mt-2 w-full">
        <Table className="scrollbar-hidden mx-auto w-full border border-gray-700 text-center font-varela">
          <TableRow className="bg-gray-700 font-bold text-gray-100 hover:bg-gray-700">
            ...
            <td className="px-0.5 text-2xs">R1</td>
            <td className="px-0.5 text-2xs">R2</td>
            <td className="px-0.5 text-2xs">R3</td>
            <td className="px-0.5 text-2xs">R4</td>
            <td className="px-0.5 text-2xs">Make Cut</td>
          ...
          {golfers
            ...
            .map((golfer, i) => (
              ...
              >
                ...
                <td className="border-gray-300 text-xs">{golfer.roundTwo}</td>
                <td className="border-gray-300 text-xs">{golfer.roundThree}</td>
                <td className="border-gray-300 text-xs">{golfer.roundFour}</td>
                <td className="border-l border-gray-300 text-xs">
                  {golfer.makeCut === 0
                    ? "-"
                    : Math.round(+(golfer.makeCut ?? 0) * 10000) / 100 + "%"}
                </td>
