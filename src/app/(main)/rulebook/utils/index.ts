 ...
 * Rulebook Utilities
 *
 * Helper functions and utilities for the rulebook module.
 */
...
/**
 * Creates a Silver tier from Gold playoff tier data
 * Used for display purposes in tables
 */
export function createSilverTier(goldPlayoffTier: Tier): Tier {
  return {
    payouts: goldPlayoffTier.payouts.slice(75) ?? [],
    points: goldPlayoffTier.points.slice(75) ?? [],
    name: "Silver",
    seasonId: goldPlayoffTier.seasonId,
    id: "silver",
    createdAt: new Date(),
    updatedAt: new Date(),
  };
...
/**
 * Sorts tiers by payout amount for consistent display order
 */
export function sortTiersByPayout(tiers: Tier[]): Tier[] {
  return tiers.sort((a, b) => (a.payouts[0] ?? 0) - (b.payouts[0] ?? 0));
}

/**
 * Gets display name for tier (converts "Playoff" to "Gold")
 */
export function getTierDisplayName(tierName: string): string {
  return tierName === "Playoff" ? "Gold" : tierName;
}

/**
 * Checks if a tier is a playoff tier
 */
export function isPlayoffTier(tierName: string): boolean {
  return tierName === "Playoff";
}

/**
 * Checks if a tier is a silver tier
 */
export function isSilverTier(tierName: string): boolean {
  return tierName === "Silver";
}

/**
 * Gets CSS classes for tier-specific styling
 */
export function getTierStyling(tierName: string): string {
  if (isPlayoffTier(tierName)) {
    return "border-l-slate-500 bg-yellow-50 bg-opacity-50";
  }
  if (isSilverTier(tierName)) {
...
/**
 * Gets the rulebook data
 */
export function getRulebookData(): RuleCategory[] {
  return [
    {
      category: "Schedule",
      rules: [
