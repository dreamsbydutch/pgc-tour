...
export const validationUtils = {
  ...
  validateStoreData: () => {
    ...
    if (!mainState.seasonTournaments?.length)
      issues.push("No tournaments loaded");
  ...
  validateTournamentTiming: () => {
    ...
    return {
      ...
      summary:
        issues.length === 0
          ? "Tournament timing is correct"
          : `${issues.length} timing issues found`,
    };
  },
};
...
export const quickActions = {
  ...
  polling: {
    ...
    status: () => {
      ...
      const shouldPoll = currentTournament
        ? shouldPollLeaderboard(currentTournament)
        : false;
...
if (typeof window !== "undefined" && process.env.NODE_ENV === "development") {
  ...
  (window as unknown as Record<string, unknown>).validateStore =
    validationUtils;
